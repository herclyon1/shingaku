...
# 学校分层（抽卡稀有度）。依据是日本受验界的既成用语与公开事实，不做编造：
# 産近甲龍＝关西私立第二集团（京都産業·近畿·甲南·龍谷），摂神追桃＝第三集团（摂南·神戸学院·追手門·桃山学院）
TIER = {
 '近畿大学': ('gold', '産近甲龍成员。报考人数连续多年全国第一，全国性知名'),
 '神戸学院大学': ('purple', '摂神追桃成员（「神」）'),
 '追手門学院大学（パートナー校制度）': ('purple', '摂神追桃成员（「追」）'),
 '桃山学院大学': ('purple', '摂神追桃成员（「桃」）。名额6人是全表最多'),
 '大阪経済大学　国際共創部　国際共創学科': ('purple', '关西老牌商科（1932年建学），就职人脉在关西金融·商界有口碑，社会评价与摂神追桃同档'),
 '京都外国語大学': ('purple', '外语单科名门，与关西外大并称关西外语双雄'),
 '天理大学': ('blue', '1925年建学的百年校，柔道等体育名门，留学生日本語教育传统悠久'),
 '山梨学院大学': ('blue', '箱根驿传常连，体育方面全国知名'),
 '流通科学大学': ('blue', '大荣创始人中内㓛1988年创立，流通·零售业界人脉'),
 '大阪経済法科大学': ('blue', '以公务员·法律资格考试辅导和亚太研究著称'),
 '日本工業大学': ('blue', '1967年建学的工科单科大学，动手实习传统，校内设工业技术博物馆'),
 'ものつくり大学': ('blue', '2001年设立的技能特化大学，建设·制造业就职有口碑'),
 '上海大学': ('purple', '中国211/双一流高校——论卡面可能是全表价值最高的卡之一，但明文只收非中国籍，对你永久锁定'),
}
TIER_DEFAULT = ('white', '无全国性分层用语覆盖，社会认知以所在地区为主——不代表教育差，代表名字不带光环')
...
N1_GAIN = {
 ...
 '大阪経済法科大学': '2年次からの在学減免が N2 の年15万 → N1 で年20万（2027年度要項）',
...
def build():
    ...
    for x in U:
        ...
        rows[-1]['tier'], rows[-1]['tier_why'] = TIER.get(x['short'], TIER_DEFAULT)
        rows[-1]['bin'] = '指定排除' in rows[-1]['why']
